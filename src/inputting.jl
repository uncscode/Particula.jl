...
function ℞T(T)
    if unit.(upreferred.(T)) == unit.(ustrip.(T) .* u"K")
        return T .|> upreferred
    elseif unit.(upreferred.(T)) == unit.(ustrip.(T) .* NoUnits)
        return T .* u"K"
    else
...
function ℞µ(µ)
    if unit.(upreferred.(µ)) == unit.(ustrip.(µ) .* u"kg" ./ u"m" ./ u"s")
        return µ .|> upreferred
    elseif unit.(upreferred.(µ)) == unit.(ustrip.(µ) .* NoUnits)
...
function ℞p(p)
    if unit.(upreferred.(p)) == unit.(ustrip.(p) .* u"kg/m/s^2")
        return p .|> upreferred
    elseif unit.(upreferred.(p)) == unit.(ustrip.(p) .* NoUnits)
        return p .* u"kg/m/s^2"
    else
...
function ℞M_wt(M_wt)
    ...
    elseif unit.(upreferred.(M_wt)) == unit.(ustrip.(M_wt) .* NoUnits)
